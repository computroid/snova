...
class TestSnovaBase(unittest.TestCase):
	...
	def init_snova(self, snova_name, **kwargs):
		...
		if not os.path.exists(sparrow_tmp_path):
			exec_cmd(
				f"git clone https://github.com/computroid/sparrow -b {SPARROW_BRANCH} --depth 1 --origin upstream {sparrow_tmp_path}"
			)
		...
		if not os.path.exists(os.path.join(self.snovaes_path, snova_name)):
			...
			exec_cmd(
				"git remote set-url upstream https://github.com/computroid/sparrow",
